function obj = retaskSensors_external(obj)
    ...
    for p = 1:P
        ...
        if any(valid)            
            % Make a fake sensor to capture the "measurement" at each
            % possible sensor location
            tempSN = obj;
            tempSN.sensors(3) = Sensor(0, p, p, tempSN, tempSN.environment);
            sensor = tempSN.sensors(3);
            sensor.hasMeasurement = true;
            tempSN.sensors(3) = sensor;            
            [~, rgbPatch, bounds] = sensor.sampleObservation();
            tempSN = tempSN.updateEstimate( ...
                                                 { rgbPatch, bounds }, ...
                                                 [0 0]);
            MI = tempSN.estimate.MI;

            rawCRMI(p) = relevance * (1+MI);
        end
    end
