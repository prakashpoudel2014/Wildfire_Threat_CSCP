function obj = retaskSensors_external_SMI_POD(obj)
    % RETASKSENSORSEXTERNAL_SMI_POD  Greedy assignment using SMI + distance,
    % excluding each sensor's own location and any already-assigned destinations.
    %
    % This is identical to retaskSensors_external_SMI, except the SMI score is
    % computed using the FULL POD-reduced covariance obj.POD.Pr and basis obj.POD.Up:
    %
    %   SMI(patch) = 0.5 * log det( I + (1/R) * Pr * (Hr' Hr) )
    %   Hr = Up(idxVec,:) where idxVec are the full-state indices inside the patch.
    %
    % No POD quantities feed back into the estimate; this only changes the SMI metric.
    ...
    % POD objects (assumed present/correct)
    Up = obj.POD.Up;      % [L×q]
    Pr = obj.POD.Pr;      % [q×q]
    q  = size(Up,2);
    ...
    SMI = zeros(1, P, 'single');
    R   = single(1);  % keep consistent with non-POD version

    for p = 1:P
        c = centers(:, p);

        % Find indices of all pixels relevant to each patch
        absXY = c + xyRel;
        ...
        if any(valid)
            % Keep the same (xIdx,yIdx) ordering as your non-POD SMI code
            % for compliance with existing behavior.
            idx = sub2ind(sz, xIdx(valid), yIdx(valid));   % linear indices into full state

            Hr = Up(idx, :);                % [m×q]
            G  = Hr' * Hr;                  % [q×q]
            A  = eye(q, 'like', Pr) + (Pr * G) / R;
            A  = 0.5 * (A + A');

            % SMI = 0.5*logdet(A) via Cholesky
            C = chol(A);
            SMI(p) = single(sum(log(diag(C))));
        end
    end
    ...
    for i = 1:numS
        ...
        % Reward: SMI minus normalized distance penalty (same structure as non-POD)
