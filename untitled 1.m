...
max_AA_temp = max(max(max(AA.Temp(:,:,:) )));
normalized_max_AA_temp =  AA.Temp/max_AA_temp;
% Reconstruction Error over Time
% error = zeros(1, nt);
% for i = 1:nt
%     error(i) = norm(Xr_spatial(:,:,i) - AA.Temp(:,:,i), 'fro') ...
%                / norm(AA.Temp(:,:,i), 'fro');
% end
error = zeros(1, nt);
for i = 1:nt
    error(i) = norm(normalized_max_Xr_spatial(:,:,i) - normalized_max_AA_temp(:,:,i), 'fro') ...
